...
var dataAPI = require('./vault_data');
var api;
...
describe("Vault Script", function(){
				...
				it("case " + item.id + " type: " + item.transactionType,function(done){
					...
					api[item.apiCall](item.data,function(err,response){
						...
						switch (item.transactionType){
							case "Create Customer" || "Update Customer":
								results[item.id].customerId = response.customerId;
								break;
							case "Create Account" || "Update Vault Account":
								results[item.id].customerId = response.vaultPaymentMethod.customerId;
								results[item.id].paymentMethodId = response.vaultPaymentMethod.paymentId;
								break;
							case "Create Customer and Account" || "Update Vault Customer and Account":
								results[item.id].customerId = response.vaultCustomer.customerId;
								results[item.id].paymentMethodId = response.assignedPaymentId;
								break;
							case "Auth - Add Customer and Account":
								results[item.id].customerId = response.transaction.vaultData.token.customerId;
								results[item.id].paymentMethodId = response.transaction.vaultData.token.paymentMethodId;
								break;
						}
